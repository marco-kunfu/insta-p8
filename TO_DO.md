...
- [x] Contraste del violeta como texto en modo oscuro en TODA la app: todos los `text-primary` que son letra o icono (chips "Add product"/"+ Add" del CreateRuleForm, tiles del overview, "AI Settings", landing, variante `link` de Button, tag-input, hovers de enlaces en field/empty/item, icono de instagram-return) pasados a `text-primary-text` (#734bfc en claro / #bcb2ff en oscuro). Verificado en el navegador: 0 elementos con #734bfc como color de texto en dark en overview, inbox, automations (incl. formulario de regla con picker de productos), ice-breakers, analytics y settings. `radio-group` conserva `text-primary` a propósito (es el relleno del punto, 3.3:1 cumple el 3:1 de componentes UI)
- [x] Reset de la base (`prisma db push --force-reset`) tras el cambio a la app real de Meta: el `user_id` del login es app-scoped y la fila vieja chocaba por `username` único. Cuenta reconectada (`marco.macias.kunfupay`, token hasta 2026-10-20). Pendiente re-vincular al vendor desde el iframe (la fila quedó con `vendor_id` null)
